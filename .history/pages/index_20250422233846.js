      ...
      <main className={styles.main}>
          ...
          <h1 className={styles.title}>
            ...
            <div className={styles.titleRow}>
              <div className={styles.titleWrapper}>
                <span 
                  className={`${styles.largeTitle} ${isTransitioning ? styles.titleExit : ''}`}
                >
                  {isSeller ? 'Sellers' : 'Buyers'}
                </span>
              </div>
              <span className={styles.divider}>/</span>
              <button 
                onClick={toggleView} 
                className={`${styles.smallTitle} ${styles.titleButton}`}
              >
                {isSeller ? 'Buyers' : 'Sellers'}
              </button>
            </div>
          </h1>
        ...
        <div className={styles.servicesGrid}>
          {services.map((service, index) => (
            <div 
              key={service.id} 
              className={`${styles.serviceCard} ${isTransitioning ? styles.cardFlip : ''}`}
